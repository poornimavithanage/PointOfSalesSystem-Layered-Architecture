...
public class PlaceOrderFormController {
    ...
    public void btnPlaceOrder_OnAction(ActionEvent actionEvent) {
        ...
        try {
            result = ordersBO.placeOrder(new OrderTM(lblId.getText(), LocalDate.now(), cmbCustomerId.getValue().getId(), cmbCustomerId.getValue().getName(),0),tblOrderDetails.getItems());
        } catch (Exception e) {
            e.printStackTrace();
        }
        if (!result){
...
//    }
    ...
    private void generateOrderId() {
        // Generate a new id
        try {
            lblId.setText(ordersBO.getNewOrderId());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
